...
def CubicBezierModule(poslist,t,draw=True):
    newposlist = []
    ...
    if(draw):
        ...
        for i in  range(len(newposlist)-1):
            pygame.draw.line(screen, BLACK, newposlist[i], newposlist[i+1], 4)
...
while not done:   
    ...
    for i in range(count):
        pygame.draw.rect(screen, BLUE, (pts[i][0]-margin, pts[i][1]-margin, 2*margin, 2*margin), 7)
        x = format(pts[i][0], '.1f')
        y = format(pts[i][1], '.1f')
